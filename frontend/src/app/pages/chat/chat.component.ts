...
export class ChatComponent implements AfterViewChecked, OnDestroy, OnInit {
  ...
  scrollToBottom(): void { try { if (this.messageContainer) this.messageContainer.nativeElement.scrollTop = this.messageContainer.nativeElement.scrollHeight; } catch (err) {} }
  ...
  async enviarAlBackend(m: string, i: number): Promise<void> { try { const r = await fetch(`${API_BASE}/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: m, historial: this.historialCompleto }) }); if (!r.body) throw new Error(); const reader = r.body.getReader(); const decoder = new TextDecoder(); let full = ''; const process = ({ done, value }: any): Promise<void> => { if (done) { this.ngZone.run(() => { this.messages[i].raw = full; const f = full.trim() || '😊 Entendido.'; this.messages[i].text = marked.parse(f) as string; this.historialCompleto.push({ role: 'assistant', content: f }); this.isTyping = false; this.guardarChatActual(); if (this.autoPlayTTS) this.speakMessage(f, i); }); return Promise.resolve(); } full += decoder.decode(value, { stream: true }); this.ngZone.run(() => { this.messages[i].raw = full; this.messages[i].text = marked.parse(full + '▌') as string; }); return reader.read().then(process); }; return reader.read().then(process); } catch { this.ngZone.run(() => { this.messages[i].text = '😔 Error de conexión.'; this.isTyping = false; }); } }
